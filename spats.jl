module SpaTs
    ...
    """
    function p3fold_refine(outdir; ybins=nothing, n_iter=5, continuity_weight=0.2, vmin_frac=0.0, vmax_frac=1.0, show_=true)
        p    = Tools.read_params(joinpath(outdir, "params.json"))
        data = Data.load_ascii(joinpath(outdir, "pulsar.debase.txt"))
        Data.zap!(data; ranges=haskey(p, "zaps") ? p["zaps"] : nothing)
        ...
        Plot.p3fold_compare(result.folded, folded_const, result.p3_per_pulse, p3, outdir;
                            bin_st=p["bin_st"], bin_end=p["bin_end"], vmin_frac=vmin_frac, vmax_frac=vmax_frac,
    ...
    """
    function p3fold_coherent(outdir; ybins=nothing, lowpass_cutoff=1/300, filter_order=6, n_groups=4, vmin_frac=0.0, vmax_frac=1.0, show_=true)
        p    = Tools.read_params(joinpath(outdir, "params.json"))
        data = Data.load_ascii(joinpath(outdir, "pulsar.debase.txt"))
        Data.zap!(data; ranges=haskey(p, "zaps") ? p["zaps"] : nothing)
    ...
    """
    function p3fold_norefine_compare(outdir; ybins=nothing, vmin_frac=0.0, vmax_frac=1.0, show_=true)
        p  = Tools.read_params(joinpath(outdir, "params.json"))
        p3 = Float64(p["p3"])
        yb = isnothing(ybins) ? Int(p["p3_ybins"]) : ybins
    ...
    function main()
        ...
        p3fold_norefine_compare(vpmout*"J2139+2242";vmin_frac=0.05, vmax_frac=1.0)
