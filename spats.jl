...
function analyse_all_2(dataroot="/home/psr/data/new/", vpmout="/home/psr/output/")
    isdir(dataroot) || error("dataroot not found: $dataroot")
    ...
    # Code 1 exact sorting for pulsar directories (reversed order)
    for psr_path in sort(filter(isdir, readdir(dataroot, join=true)), rev=true)
        psr = basename(psr_path)

        # Code 1 exact sorting for observation subdirectories (reversed order)
