module Plot
    ...
    function lrfsdwa(data, outdir;
        ...
        # Profile boczne
        average_x = sum(da, dims=1)[1, :]  # wzdłuż kolumn
        average_y = sum(da, dims=2)[:, 1]  # wzdłuż wierszy
        ...
        grid = fig.add_gridspec(6, 3, height_ratios=[1, 0.1, 4, 0, 0, 0])

        # Górny panel: profil poziomy
        ax_top = fig.add_subplot(grid[0, 1:3])
        ax_top.minorticks_on()
        ax_top.plot(xrange, average_x, color="grey")
        ax_top.set_xlim(xrange[1], xrange[end])
        ax_top.set_yticks([])
        ax_top.set_xlabel("Longitude [bins]")

        # Lewy panel: profil pionowy
        ax_left = fig.add_subplot(grid[2, 0])
        ax_left.minorticks_on()
        ax_left.plot(average_y, yrange, color="grey")
        ax_left.set_ylim(yrange[1], yrange[end])
        ax_left.set_xticks([])
        ax_left.set_ylabel("Pulse number")

        # Główny panel
        ax_main = fig.add_subplot(grid[2, 1:3])
        im = ax_main.imshow(norm_da,
                            origin="lower",
                            cmap=cmap,
                            interpolation="none",
                            aspect="auto",
                            vmin=0.0,
                            vmax=1.0)
        ax_main.tick_params(labelleft=false, labelbottom=false)

        # Colorbar w nowej, niezależnej osi
