...
=#
    ...
    function process_psrfit_files(base_dir::String, output_dir::String; name_mod::Union{String, Nothing}=nothing)
        ...
        # Step 10: Plot
        Plot.single(combined_data, output_subdir;darkness=0.5,bin_st=p["bin_st"],bin_end=p["bin_end"],start=p["pulse_start"],number=(p["pulse_end"]-p["pulse_start"]) ,name_mod="J0820Mac",show_=true)
        Plot.lrfs(combined_data, output_subdir;darkness=0.1,start=p["pulse_start"],bin_st=p["bin_st"],bin_end=p["bin_end"],name_mod="J0820Mac",change_fftphase=false,show_=true)
        Plot.average(combined_data, output_subdir;bin_st=p["bin_st"],bin_end=p["bin_end"],number=(p["pulse_end"]-p["pulse_start"]), name_mod="J0820Mac",show_=false)
#=
