...
function analyse_all(dataroot="/home/psr/data/new/", vpmout="/home/psr/output/";
    ...
    # Code 1 style: filter(isdir, readdir(..., join=true)) and reversed sorting if downward=true
    psr_paths = sort(filter(isdir, readdir(dataroot, join=true)), rev=downward)
    psr_dirs  = basename.(psr_paths)
    isempty(psr_dirs) && (@warn "No pulsar directories found in $dataroot"; return)
    for psr in psr_dirs
        psr_path = joinpath(dataroot, psr)
        
        # Code 1 style: sort with rev=true to select the last observation directory
        obs_dirs = sort(filter(isdir, readdir(psr_path, join=true)), rev=true)
        if isempty(obs_dirs)
            @warn "No observation subdirectory for $psr, skipping"
            continue
        end
        # obs_dirs[1] is now the last/latest observation folder
        indir  = obs_dirs[1] * "/"
        obs_name = basename(obs_dirs[1])
        outdir = joinpath(vpmout, psr * "_16")
        if isdir(outdir)
            println("=== Skipping $psr (outdir exists) ===")
            continue
        end
        p3_str = get(p3_map, psr, "unknown")
        println("=== Processing $psr (obs: $obs_name), P3 = $p3_str ===")
        try
