module Data
    ...
    function process_psrdata_16(indir, outdir; files=nothing, outfile="pulsar.spCf16", params_file="params.json")
        ...
        Plot.single(da_lo, outdir; darkness=0.7, number=p["nsubint"], bin_st=p["bin_st"], bin_end=p["bin_end"], start=1, name_mod="low", show_=true)

        # single pulses - high
        da_hi = Data.load_ascii(replace(high_filename, ".high"=>"_high_debase.txt"))
        Plot.single(da_hi, outdir; darkness=0.7, number=p["nsubint"], bin_st=p["bin_st"], bin_end=p["bin_end"], start=1, name_mod="high", show_=true)
